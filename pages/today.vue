...
const state = reactive({
  map: {
    ...
    center: [77.2283935546875, 28.66528620762573] as number[],
    zoom: 7,
    maxZoom: 22,
...
async function getMapData(tempMap: mapboxgl.Map) {
  console.log("data will show here");

  getalldata();
  async function getalldata() {
    data.mapEvent = await $fetch("http://localhost:3001/map");
  }
  console.log("getallfunction", data.mapEvent);
...
async function submit() {
  ...
  let response = await $fetch("http://localhost:3001/map", {
    method: "POST",
    // body: payload,
    body: info.data1,
  });
  ...
  await $fetch("http://localhost:3001/map/polygon", {
    method: "POST",
    body: formData,
  }).catch((err) => alert(err));
}
